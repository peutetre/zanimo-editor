...
function store(id, name, val, state, t, isPublic) {
    return _gistSet.set(name, gistObj(id, name, val, state, t, isPublic));
}

function storef(name, val, state) {
    return function (g) {
        return store(g.id, name, val, state, g.updated_at, g['public']);
    };
}
...
gist.create = function (name, val, isPublic) {
    ...
    return user.createGist(name, val, isPublic).then(storef(name, val, "sync"),
        function (err) {
