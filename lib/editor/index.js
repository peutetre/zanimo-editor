...
editor.init = function () {
    ...
    (new MobileButton.Touchend({
        el: $.$("button.sign", $editor), f: editor.onSign
    })).bind();
    (new MobileButton.Touchend({
        el: $.$("button.logout", $editor), f: editor.onLogout
    })).bind();
    (new MobileButton.Touchend({
...
editor.actions = {
    ...
    defaultAction : function (v, id) {
        if(select.isOption(id)){
            current_type = select.getSection(id);
            return editor.set(v, current_type);
        }
        else {
            select.set(current);
            return Q();
...
editor.onSelect = function (val, idx) {
    softkeyboard.hide();
    (function () {
        switch(val) {
            ...
            default:
                return editor.actions.defaultAction(val, idx);
        }
