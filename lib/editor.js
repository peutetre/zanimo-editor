...
editor.init = function () {
    ...
    editor.set(data.first(data.types[0]), data.types[0]);
...
editor.onSave = function (evt) {
    ...
    $hidden.focus();
    try {
        data.save(current, type, _editor.getValue());
...
editor.onSelect = function (evt) {
    ...
    if (name === "New Local Script" /* TODO || name === "New Gist Script"*/) {
        var input = window.prompt("Animation name:", "...");
            input = input ? input.trim() : null;
        if (input in commands || !input) {
            notification.fail("Name is invalid...");
            setTimeout(function () { $select.value = current; }, 30);
            return;
        }
        data.create(input, commands[name]);
        editor.set(input, commands[name]);
    } else if(name === "Sync Zanimo Gists" || name === "New Gist Script" /* remove when impl.*/ ) {
        notification.fail("✗ not implemented...", 1000);
        setTimeout(function () { $select.value = current; },30);
    }
    else {
...
editor.set = function (name, t) {
    ...
    _editor.setValue(data.get(current, type) || "");
};
