...
editor._select = function (evt, commands, sections) {
    ...
    switch(name) {
        ...
        case "About":
            return alert.show('About', '<b>Zanimo-editor v'
                    + pkg.version
                    + '</b> featuring <b>Zanimo v'
                    + pkg.dependencies.zanimo
                    + '</b> and <b>Q v'
                    + pkg.dependencies.q + '</b><br>'
                    + 'Create Animations with styled CSS3 transitions right in your device!')
                .then(function () { return editor.set(name, current_type); });
        default:
