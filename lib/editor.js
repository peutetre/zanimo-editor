...
editor.onSelect = function (evt) {
    ...
    try {
        $hidden.focus();
        if (name === "New Local Script" || name === "New Gist Script") {
            ...
            data.create(input, commands[name])
                .then(function (id) {
                    editor.set(id, commands[name]);
                });
        } else if(name === "Sync Zanimo Gists") {
            data.sync()
                .then(function () {
                     return data.first(data.types[0]).then(function (d) {
                        return editor.set(d, data.types[0]);
                    });
                })
                .then(notification.successƒ("sync done"), notification.fail);
        }
        else {
...
editor.set = function (name, t) {
    return data.get(name, t).then(function (code) {
        ...
        if(err.error && err.request) {
            notification.fail("✗ " + err.error + " " + t + " " + name + " " + err.request.statusText.toLowerCase());
        }
        else {
            notification.fail(err);
        }
